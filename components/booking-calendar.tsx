...
export default function BookingCalendar() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl md:text-3xl font-bold mb-6 text-center">Meal Booking Calendar</h1>
      <div className="space-y-6">
            ...
            components={{
              DayContent: ({ date }) => (
                <div className="flex flex-col items-center justify-center h-full">
                  ...
                  {isDateEnabled(date) && (
                    <span className="text-xs font-semibold">{getMealCount(date)}</span>
                  )}
                </div>
              ),
            }}
        ...
        <Dialog>
          <DialogTrigger asChild>
            ...
            >
              Book Meals for {selectedDate ? format(selectedDate, 'MMM dd, yyyy') : 'Selected Date'}
            </Button>
          </DialogTrigger>
          <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
              <DialogTitle>Book Meals for {selectedDate ? format(selectedDate, 'MMMM d, yyyy') : 'Selected Date'}</DialogTitle>
              <DialogDescription>Select the meals you want to book for this day.</DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4">
              {selectedDate && ['morning', 'noon', 'evening'].map((meal) => (
                <div key={meal} className="flex items-center space-x-2">
                  <Checkbox
                    id={meal}
                    checked={bookings[format(selectedDate, 'yyyy-MM-dd')]?.[meal as keyof MealBooking] || false}
                    onCheckedChange={() => handleBookingChange(meal as keyof MealBooking)}
                  />
